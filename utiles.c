...
static double matrix_get_value(const Matrix* matrix, int row, int col) {
...
Matrix* add_matrices(Matrix* A, Matrix* B) {
    ...
    /* Cas mixte dense/CSR : le resultat est dense car les appels aval
       attendent une Matrix classique. Les entrees CSR ne sont jamais
       densifiees avant ce calcul ponctuel. */
    for (int i = 0; i < A->rows; ++i) {
        for (int j = 0; j < A->cols; ++j) {
            C->data[i][j] = matrix_get_value(A, i, j) + matrix_get_value(B, i, j);
        }
    }
...
Matrix* soustraction(Matrix* A, Matrix* B) {
    ...
    /* Calcul dense du residu, mais lecture de A possible depuis une vue CSR.
       Cette allocation existait deja dans _compute_fit ; elle n'est pas faite
       par le chargeur et ne materialise pas les slices a l'import. */
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            result->data[i][j] = matrix_get_value(A, i, j) - matrix_get_value(B, i, j);
        }
    }
